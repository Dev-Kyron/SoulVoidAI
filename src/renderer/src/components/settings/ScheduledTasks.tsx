...
import { Plus, Play, Trash2, Clock, RefreshCw, AlarmClock } from 'lucide-react'
import { CollapsibleSection } from './CollapsibleSection'
import { EmptyState, Toggle } from '../common/ui'
import { vs } from '../../lib/bridge'
import { useUiStore } from '../../store/useUiStore'
import { cn } from '../../lib/utils'
import type { ScheduledTask, ScheduleKind } from '@shared/types'
    ...
    <div className="glass-soft space-y-1.5 rounded-lg p-2.5">
      <div className="flex items-start gap-2">
        ...
        <div className="min-w-0 flex-1">
          <p className="truncate text-[12px] font-medium text-slate-100">{task.name}</p>
          ...
          <p className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-0.5 text-[10px] text-slate-500">
            ...
            {task.lastRun && (
              <span>last: {new Date(task.lastRun).toLocaleString([], { dateStyle: 'short', timeStyle: 'short' })}</span>
            )}
          </p>
        </div>
...
function NewTaskForm({ onCreated }: { onCreated: () => void }): JSX.Element {
  ...
  const placeholder =
    kind === 'daily' ? '09:00' : kind === 'interval' ? '60' : new Date(Date.now() + 60_000).toISOString().slice(0, 16)

  const submit = async (): Promise<void> => {
    if (!name.trim() || !prompt.trim() || !value.trim()) {
      pushToast('error', 'Name, prompt and schedule are all required.')
      return
    }
    setBusy(true)
    try {
      const normValue =
        kind === 'once' && value.length === 16
          ...
          : value
      await vs.scheduler.add({ name: name.trim(), prompt: prompt.trim(), scheduleKind: kind, scheduleValue: normValue })
      setName('')
      setPrompt('')
      pushToast('success', `Scheduled "${name.trim()}".`)
      onCreated()
    } finally {
      setBusy(false)
    }
  ...
  return (
    <div className="glass-soft space-y-1.5 rounded-lg p-2.5">
      ...
      <input
        ...
        placeholder="Name (e.g. Morning GitHub digest)"
        className={cn(FIELD, 'w-full')}
      />
      <textarea
        value={prompt}
        onChange={(e) => setPrompt(e.target.value)}
        placeholder="Prompt to send headlessly (e.g. List my open GitHub issues assigned to me, ranked by priority.)"
        rows={3}
      ...
      <div className="flex gap-1.5">
        <select
          ...
          onChange={(e) => {
            ...
            setKind(k)
            setValue(k === 'daily' ? '09:00' : k === 'interval' ? '60' : new Date(Date.now() + 60_000).toISOString().slice(0, 16))
          }}
          className={FIELD}
        >
          <option value="daily" className="bg-void-700">Daily at…</option>
          <option value="interval" className="bg-void-700">Every N minutes</option>
          <option value="once" className="bg-void-700">Once at…</option>
        </select>
        <input
...
export function ScheduledTasks(): JSX.Element {
  ...
  return (
    <CollapsibleSection
      ...
      hint='Prompts that fire on a schedule. Headless — uses the active provider/model. Results show here and as a system notification.'
    >
      <div className="space-y-2">
        ...
        {tasks.length === 0 ? (
          ...
          tasks.map((task) => (
            <TaskRow key={task.id} task={task} onChange={() => void refresh()} />
          ))
        )}
      </div>
    </CollapsibleSection>
